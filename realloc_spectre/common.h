...
static void init() {
    ...
    shared_mem = mmap(NULL, SHMEM_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, shmem_fd, 0);
}
...
static uint64_t virt2phys(void* virt) {
    ...
    return ((pfn & 0x7FFFFFFFFFFFFFull) << 12ull) | (((uintptr_t)virt) & 0xfffull);
}

// adjust this if you want to use the oracle instead
#define virt2phys_attack virt2phys

_Static_assert(SHMEM_SIZE >= (PAGES2 + 4095) / 256, "shared memory is too small to store the results");
