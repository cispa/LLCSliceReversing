...
private:
  ...
  /**
   * Search the page frame number (pfn) with the given pattern in the physical
   * address space
   *
   ...
   */
  uint64_t search_pfn(std::vector<int> &pattern);

  /**
   * Count the number of faults for the given pfn and pattern
   */
  uint64_t count_faults(size_t pfn, std::vector<int> &pattern);
   ...
   * @param pattern_length The length of the pattern in cache lines (64 bytes)
   */
  void record_pattern(void *address, std::vector<int> &pattern,
                      int pattern_length);
...
public:
  ...
  /**
   * @brief Provides a timing-based oracle for the translation of virtual to
   * physical addresses
   *
   * @param address The virtual address to translate
   * @param pattern_length_bytes The length of the pattern to use in the oracle
   * in bytes
   */
