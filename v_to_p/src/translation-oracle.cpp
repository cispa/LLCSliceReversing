...
uint64_t TranslationOracle::count_faults(size_t pfn,
...
TranslationOracle::record_pattern(void *address, std::vector<int> &pattern,
                                  int pattern_length) {
  // Get the start time
  ...
  std::cout << TAG_OK << "Took " << duration << " seconds, " << fault_ctr
            << "faults" << std::endl;
}
  ...
  for (uint64_t current_pfn = 0; current_pfn < upper_limit; ++current_pfn) {
    ...
    if (current_pattern_correct) {
      ...
      if (in_suspected_range(physical_address)) {
        ...
        // std::cout << TAG_OK << "Pattern correct for physical address: " <<
