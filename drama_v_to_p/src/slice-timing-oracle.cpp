...
void filter_timings(std::vector<std::vector<uint64_t>> &measurement,
                    int threshold) {
  ...
  for (auto &vec : measurement) {
    for (auto it = indicesToRemove.rbegin(); it != indicesToRemove.rend();
         ++it) {
...
address_t get_random_address(size_t number_of_bytes) {
  uint64_t *mem =
      (uint64_t *)mmap(0, number_of_bytes, PROT_WRITE,
  ...
  address.physical_address =
      (void *)utils_get_physical_address((size_t)address.virtual_address);
  return address;
}

SliceTimingOracle::SliceTimingOracle(CoreConfig core_config) {
  this->core_config =
      core_config; // std::cout << "Determining Treshold" << std::endl;
  // determine_treshold();
  // this->threshold = 268;
...
/**
 * Returns the distance of the minumum value in the vector to the second lowest
 * value
 */
...
void SliceTimingOracle::record_timings(
    void *address, std::vector<std::vector<uint64_t>> &measurements,
    int repetitions) {
  for (size_t i = 0; i < repetitions; i++) {
    for (size_t c = 0; c < this->core_config.num_slices; c++) {
      ...
      // "rax"); asm volatile("mfence\n" : : :);
      // // asm volatile("movq (%0), %%rax\n" : : "r"(address) : "rax");
      // size_t start = __rdtsc_begin();
      // // asm volatile("clflush 0(%0)\n" : : "c"(address) : "rax");
      // asm volatile(".rept 1\nmovq (%0), %%rax\nclflush 0(%0)\n.endr\n" : :
      // "c"(address) : "rax"); size_t end = __rdtsc_end(); measurement = end -
      // start;
...
bool SliceTimingOracle::test_quality_timings(
...
void SliceTimingOracle::dump_timings(
    std::vector<std::vector<uint64_t>> &measurements, int slice) {
...
uint64_t __attribute((aligned(4096))) SliceTimingOracle::oracle(void *addr) {
  int repetitions = 30;
  std::vector<std::vector<uint64_t>> measurements(
      this->slices, std::vector<uint64_t>(repetitions, 0));
  ...
  return std::distance(medians.begin(),
...
uint64_t SliceTimingOracle::oracle_min(void *addr) {
  ...
  uint64_t min_idx =
      std::distance(mins.begin(), std::min_element(mins.begin(), mins.end()));
...
public:
  LogisticRegression(const std::vector<double> &coeffs)
      : coefficients(coeffs) {}
...
uint64_t SliceTimingOracle::oracle_mean(void *addr, int correct_slice) {
  ...
  for (size_t i = 0; i < this->slices; i++) {
    float mean = (float)std::accumulate(measurements[i].begin(),
                                        measurements[i].end(), 0.0) /
                 (float)measurements[i].size();
    means[i] = mean;
  }

  // std::cout << "Means: " << means << std::endl;
  return std::distance(means.begin(),
...
uint64_t SliceTimingOracle::oracle_info(void *addr, int correct_slice) {
  ...
  std::vector<std::vector<uint64_t>> measurements(
      this->slices, std::vector<uint64_t>(3000, 0));
  std::vector<uint64_t> medians(this->slices, 0);
  record_timings(addr, measurements, repetitions);
  for (size_t i = 0; i < this->slices; i++) {
    std::vector<uint64_t> measures = measurements[i];
  ...
  int measured_slice = std::distance(
      medians.begin(), std::min_element(medians.begin(), medians.end()));
  if (measured_slice == correct_slice) {
    std::cout << TAG_OK;
  } else {
    std::cout << TAG_FAIL;
  }
  std::cout << "Medians: " << medians << "->" << measured_slice << "~"
            << correct_slice << std::endl;
