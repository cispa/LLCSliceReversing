...
int verify_pattern(size_t pfn, int *pattern, int pattern_length, int fault_tolerance)
{
...
uint64_t TranslationOracle::count_faults(size_t pfn, int *pattern, int pattern_length)
{
    uint64_t physical_address = pfn << 12;
    uint64_t current_address;
    uint64_t fault_counter = 0;
...
void __attribute__((aligned(4096))) TranslationOracle::record_pattern(void *address, int *pattern, int pattern_length)
{
    // Get the start time
...
#endif
        // int current_slice = this->slice_oracle.oracle_dump(current_address.virtual_address, correct_slice);
...
#endif
        // std::cout << current_address.physical_address << ", " << current_slice << " correct " << correct_slice << std::endl;
#ifdef DEBUG
...
void* TranslationOracle::search_pfn(int *pattern, int pattern_length)
{
    ...
    for (uint64_t current_pfn = 0; current_pfn < upper_limit; ++current_pfn)
    {
        ...
        int current_pattern_correct = verify_pattern(current_pfn, pattern, pattern_length, 0);
