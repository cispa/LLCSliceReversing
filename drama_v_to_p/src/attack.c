...
size_t get_dram_row(size_t phys_addr) { return phys_addr >> 18; }
...
void *find_virt_in_set(void *mapping, uint64_t mapping_size,
                       void *mapping_physical, int set) {
  size_t base_set;
  size_t base_row;
  uint64_t offset = 0;
  void *base;
  ...
  do {
    ...
    // NOTE: The row shouldn't be the same for sender and receiver because the
    // buffers we use are rather large and distinct so should fill entire rows.
    // If that is not the case we can just rely on luck here. There are many
    // rows...
  } while (base_set != set);
  printf("virt %p phys %p\n", base, phys);
  printf("base set %d base_row %d\n", base_set, base_row);
...
void attack(char *sendfile, char *outfile, int nbytes, void *mapping,
            uint64_t mapping_size, void *mapping_physical) {
  printf("mapping: %p, mapping_size: %p, mapping_physical: %p\n", mapping,
         mapping_size, mapping_physical);
#ifdef DEBUG_ULTRA_FAST_MODE
...
#define SENDING_SET 0
  ...
  if (sendfile) {
    ...
    // NOTE: This is super important as framework pins us to the same core
    // otherwise
    pin_to_core(0);
    void *base =
        find_virt_in_set(mapping, mapping_size, mapping_physical, SENDING_SET);
